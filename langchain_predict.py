...
# LANGCHAIN-OPENAI Key
openai_llm = OpenAI( api_key='api-key-here')
...
def submit():
    ...
    ticket_output = openai_llm(prompt=prompt1)
    print(ticket_output)
    
    #index_equal_sign = ticket_output.index('=')
    #list_part = ticket_output[index_equal_sign + 1:].strip()
    ...
    # It is not always consistent to appear in the table, since the output is a str that needs to be converted as a list.
    rationale_output = openai_llm(prompt=prompt2)
    print(rationale_output)
    print(type(rationale_output))

    # Reorganizing the str for list transformation
    cleaned_str = rationale_output.replace('[', '').replace(']', '')
    print(cleaned_str)
    list_items = cleaned_str.split(', ')
    print(list_items)
    rationale_list = [item if item.endswith('.') else item + '.' for item in list_items]
